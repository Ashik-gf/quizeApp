...
const ResultPages = () => {
    ...
    const { quiz, attempts, stats } = quizAttempts;
    ...
    useEffect(() => {
        const fetchQuizAttempts = async () => {
            try {
                setLoading(true);
                const response = await api.get(`${import.meta.env.VITE_SERVER_BASE_URL}/api/quizzes/${quizeId}/attempts`);
                setQuizAttempts(response.data.data);
                // setCorrectAnswers(response.data.data.attempts.correct_answers);
                // setSubmittedAnswers(response.data.data.attempts.submitted_answers);
    ...
    return (
        <div className="flex min-h-screen overflow-hidden">
            ...
            {/* <!-- Left side --> */}
            {
                isLoading ? (<div className="max-h-screen overflow-hidden hidden lg:flex lg:w-1/2 bg-primary flex-col justify-center p-12 relative">
                    <div className="w-full h-full flex justify-center items-center">
                        <p className='text-center'> Result Data loading......</p>
                    </div>

                </div>) : Object.keys(quizAttempts).length > 0 && (<div className="max-h-screen overflow-hidden hidden lg:flex lg:w-1/2 bg-primary flex-col justify-center p-12 relative">
                    <div>
                        <div className="text-white">
                            <div>
                                <h2 className="text-4xl font-bold mb-2">{quiz?.title || "Wait"}
                                </h2>
                            ...
                            <div className="my-6 flex items-center  ">
                                <div className="w-1/2">
                                    <div className="flex gap-6 my-6">
                                        ...
                                        <div>
                                            <p className="font-semibold text-2xl my-0">{"rightAns"}</p>
                                            <p className="text-gray-300">Correct</p>
                                        </div>

                                        <div>
                                            <p className="font-semibold text-2xl my-0">{"wrongAns"}</p>
                                ...
                                <div className="w-1/2 bg-primary/80 rounded-md border border-white/20 flex items-center p-4">
                                    <div className="flex-1">
                                        <p className="text-2xl font-bold">{"getingMark"}/{"quiz.total_marks"}</p>
                                        <p>Your Mark</p>
                                    </div>
                                    <div>
